...
class ActivityPod : AppCompatActivity(), GameAdapter.Listener {
    lateinit var bindPod : ActivityPodBinding
    private var db = Firebase.firestore
    private  val adapter = GameAdapter(this)

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        bindPod = ActivityPodBinding.inflate(layoutInflater)
        setContentView(bindPod.root)
        supportActionBar?.setDisplayHomeAsUpEnabled(true)
        supportActionBar?.title = "Подвижные игры"
    ...
    private fun init() = with(bindPod) {
        rvPod.layoutManager = LinearLayoutManager(this@ActivityPod)
