...
class ActivitySploch : AppCompatActivity(), GameAdapter.Listener {
    lateinit var bindSploch : ActivitySplochBinding
    private var db = Firebase.firestore
    private  val adapter = GameAdapter(this)

    override fun onCreate(savedInstanceState: Bundle?){
        super.onCreate(savedInstanceState)
        bindSploch = ActivitySplochBinding.inflate(layoutInflater)
        setContentView(bindSploch.root)
        supportActionBar?.setDisplayHomeAsUpEnabled(true)
        supportActionBar?.title = "Игры на сплочение"
    ...
    private fun init() = with(bindSploch) {
        rvSploch.layoutManager = LinearLayoutManager(this@ActivitySploch)
