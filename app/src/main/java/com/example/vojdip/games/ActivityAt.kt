...
class ActivityAt : AppCompatActivity(), GameAdapter.Listener {
    lateinit var bindAt: ActivityAtBinding
    private val db = Firebase.firestore
    private val adapter = GameAdapter(this)

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        bindAt = ActivityAtBinding.inflate(layoutInflater)
        setContentView(bindAt.root)
        supportActionBar?.setDisplayHomeAsUpEnabled(true)
        supportActionBar?.title = "Игры на внимание"
    ...
    private fun init() = with(bindAt) {
        rvAt.layoutManager = LinearLayoutManager(this@ActivityAt)
