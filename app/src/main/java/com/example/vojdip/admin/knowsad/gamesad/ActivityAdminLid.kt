...
import com.example.vojdip.databinding.ActivityAdminLidBinding
...
class ActivityAdminLid : AppCompatActivity() , GameAdapter.Listener {

    lateinit var bindAdminLid : ActivityAdminLidBinding
    private var db = Firebase.firestore
    private  val adapter = GameAdapter(this)

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        bindAdminLid = ActivityAdminLidBinding.inflate(layoutInflater)
        setContentView(bindAdminLid.root)
        init()

        bindAdminLid.bnBackLid.setOnClickListener {
            val intent = Intent(this, ActivityAdminGames::class.java)
            startActivity(intent)
            this.finish()
        }
    }
    private fun init() = with(bindAdminLid){
        rvLid.layoutManager = LinearLayoutManager(this@ActivityAdminLid)
        ...
        n.get(AggregateSource.SERVER).addOnCompleteListener { task ->
            if (task.isSuccessful) {
                ...
                for (i in 1..snapshot.count) {
                    val refed = db.collection("Knows").document("Games").collection("Lid").document(i.toString())
